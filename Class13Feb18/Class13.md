...
(hereafter referred to as “dex”), a common steroid, on airway smooth
...
- **countData**: a table with genes as rows and samples/experiments as
  columns, = **colData**: metadata about the columns (i.e. samples) in
...
We will go row by row (each gene) to see if the average value in control
columns is different than the average value in treated columns.
...
Using the `rowMeans()` function like we did above is a more robust
approach than just using `rowSums()` and dividing by an arbitrary value.
...
The `arr.ind` argument tells us which genes and samples have zero
counts. We will look at the rows with zero counts.
...
> Q9. Q9. Using the `down.ind` vector above can you determine how many
...
We do not know if the fold change value is statistically significant, so
...
`BiocManager::install("AnnodationDbi")`
...
change values); and the gene set to check overlap for.
...
```
    ...
    Info: Working in directory /Users/saketchodavarapu/Documents/BIMM143/bimm143/Class13Feb18
